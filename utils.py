...
class DataAugmentation:
    ...
    def transform_triplets(self, img, gt1, gt2):
        ...
        gt1 = TF.resize(gt1, self.img_size)

        gt2 = TF.to_pil_image(gt2)
        gt2 = TF.resize(gt2, self.img_size)
        ...
        if self.with_random_crop and random.random() > 0.5:
            ...
            img = TF.resized_crop(
                img, i, j, h, w, size=self.img_size)
            gt1 = TF.resized_crop(
                gt1, i, j, h, w, size=self.img_size)
            gt2 = TF.resized_crop(
                gt2, i, j, h, w, size=self.img_size)
...
class Rain100HDataset(Dataset):
    ...
    def __init__(self, root_dir, img_size, suff='.png', is_train=True):
        self.root_dir = root_dir
        if isinstance(root_dir, (tuple, list)):
            self.dirs = []
            for single_dir in root_dir:
                self.dirs += glob.glob(os.path.join(single_dir, '*'+suff))
        else:
    ...
    def __getitem__(self, idx):
        ...
        gt2 = np.uint8(np.abs(np.int16(img_mix) - np.int16(gt1)))
...
def get_loaders(args):
    ...
    if args.dataset == 'dogs':
        ...
        img_size = 512 if 'unet' in args.net_G else [320, 480]

        training_set = Rain100HDataset(
            # root_dir=r'/mnt/190/a/cct/rain_streak/dataset/RainTrainH/norain/',
            root_dir=[r'/mnt/190/a/cct/rain_streak/dataset/RainTrainH/norain/',
                        r'/mnt/190/a/cct/rain_streak/testset/R100H/norain'],
            img_size=img_size, suff='.png', is_train=True)
        val_set = Rain100HDataset(
            root_dir=r'/mnt/190/a/cct/rain_streak/testset/R100H/norain',
            img_size=img_size, suff='.png', is_train=False)
...
to_gray = lambda x: (x[:,0:1]*65.481+x[:,1:2]*128.553+x[:,2:3]*24.966)/255+16
